...
const ProduksiNPKPage = ({ plant }: ProduksiNPKPageProps) => {
  ...
  const currentMonthProduksi = useMemo(() => {
    ...
    const totalOnspek = thisMonthData.reduce((sum, item) => sum + parseNumber(item.totalOnspek), 0);
    const totalOffspek = thisMonthData.reduce((sum, item) => sum + parseNumber(item.totalOffspek), 0);
    const total = thisMonthData.reduce((sum, item) => sum + parseNumber(item.total), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Produksi Bulan Ini */}
      <div className={`rounded-xl p-5 text-white shadow-md ${plant === "NPK1" ? "bg-gradient-to-r from-primary-500 to-primary-600" : "bg-gradient-to-r from-green-500 to-green-600"}`}>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-4 md:gap-6">
            <div className="bg-white/20 rounded-lg px-4 py-2 backdrop-blur-sm">
              ...
              <p className="text-xl font-bold">{formatNumber(currentMonthProduksi.totalOnspek)} <span className="text-sm font-normal">Ton</span></p>
            </div>
            <div className="bg-white/20 rounded-lg px-4 py-2 backdrop-blur-sm">
              <p className="text-white/80 text-xs uppercase">Offspek</p>
              <p className="text-xl font-bold">{formatNumber(currentMonthProduksi.totalOffspek)} <span className="text-sm font-normal">Ton</span></p>
            </div>
            <div className="bg-white/20 rounded-lg px-4 py-2 backdrop-blur-sm">
